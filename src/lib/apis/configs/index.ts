...
export const getLMStudioConfig = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/lm_studio`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const setLMStudioConfig = async (token: string, config: object) => {
	let error = null;
